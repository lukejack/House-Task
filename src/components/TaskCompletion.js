    ...
    submit() {
        ...
        if (this.state.selectedTask !== undefined) {
            ...
            tools.post('/post/taskcomplete', this, (data, stateRef) => {
                ...
                if (data.success) {
                    //Sync our data with the server
                    stateRef.props.addCompletion(data.completion);
                }
                //Stop loading and go to house stats
                this.setState({ loading: false });
                this.props.pageChange({ target: { value: 'housestats' }, preventDefault: () => { } });
            }, 'houseId=' + this.props.houseId + '&taskId=' + ((this.props.tasks.length === 1) ? this.props.tasks[0]._id : this.state.selectedTask) + '&description=' + this.state.inputText);
        }
}
...
taskCreationFinished(tasks) {
    ...
    tools.post('/post/taskadd', this, (data, stateRef) => {
        if (data.success) {
            //Sync our data with the server
            this.props.addTasks(data.tasks);
            this.setState({ newTasks: false, loading: false });
        }
...
render() {
    ...
    if (this.state.loading) return (<div style={spinner_css}><Loader color={'#000000'} /></div>);
